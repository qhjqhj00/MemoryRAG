...
class Reranker:
    def __init__(
        ...
        chunk_size: int = 1024):
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, cache_dir="/share/shared_models")
        self.model = AutoModelForSequenceClassification.from_pretrained(
            model_name, cache_dir="/share/shared_models", device_map={'': "cuda"}).eval()

        self.chunk_size = chunk_size
        self.spliter = TextSplitter.from_tiktoken_model("gpt-3.5-turbo")
    ...
    def __call__(
        ...
        chunks = self.spliter.chunks(context, self.chunk_size)
        reranker_input = [[query, chunk] for chunk in chunks]
        
        inputs = self.tokenizer(
            reranker_input, padding=True, truncation=True, return_tensors='pt', max_length=512)
